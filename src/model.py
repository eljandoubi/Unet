...
class UNET(nn.Module):
    def __init__(
        ...
        for idx, (metadata, type) in enumerate(self.encoder_config[::-1]):
            ...
            concat_num_channels = (
                out_dim + enc_out_channels if self.skip_connection else out_dim
            )
        ...
        concat_num_channels = (
            starting_channel_size * 2 if self.skip_connection else starting_channel_size
        )
        self.decoder_config.append(
            ((concat_num_channels, starting_channel_size), "residual")
        )
...
if __name__ == "__main__":
    m = UNET().cuda()
    tn = torch.randn(5, 3, 32, 32, device="cuda")
    out = m(tn)
    print(out.shape)
