...
class LocalLogger:
    def __init__(self, path_to_log_folder, filename="train_log.pkl"):
...
def train(
    ...
    path_to_experiment = os.path.join(".", experiment_name)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        progress_bar = tqdm(
            range(len(train_dataloader) // gradient_accumulation_steps),
        ...
        for images, targets in train_dataloader:
            with accelerator.accumulate(model):
                ...
                if accelerator.sync_gradients:
                    ...
                    accuracy_gathered = accelerator.gather_for_metrics(
        ...
        if epoch_test_loss < best_test_loss:
            ...
            accelerator.save_model(
                model,
                os.path.join(path_to_experiment, "best_checkpoint"),
                safe_serialization=False,
            )

        accelerator.save_model(
            model,
            os.path.join(path_to_experiment, "last_checkpoint"),
            safe_serialization=False,
        )
